...
// Let's generate tooth 0 flank points of Pinion, and mating flank points of Gear
// for each slice s:
const numSlices = 10;
const ptsPerFlank = 10;
const R_tool = 1.5 * b;

function getFlankPts(opt, isGear) {
    ...
    for (let s = 0; s <= numSlices; s++) {
        ...
        const spiralAngle = Math.asin(Math.max(-0.99, Math.min(0.99, sin_spiral)));
        ...
        // Flank 1: ang = centerAngle - pt.theta
        // Flank 2: ang = centerAngle + pt.theta
        const f1 = [], f2 = [];
...
const pSlices = getFlankPts({ z: z1, delta: delta1, hand: -1, ha_e: geom.hae1, hf_e: geom.hfe1, sn_e: geom.sne1 }, false);
const gSlices = getFlankPts({ z: z2, delta: delta2, hand: 1, ha_e: geom.hae2, hf_e: geom.hfe2, sn_e: geom.sne2 }, true);

// In 3D:
// Let's test at a sequence of Pinion angles a from -0.05 to +0.05 rad:
// For each a, what is the clearance at Heel (s=0), Mid (s=5), and Toe (s=10)?
console.log('Pinion Angle sweep (deg) -> Clearances at Heel (s=0), Mid (s=5), Toe (s=10):');
console.log('Angle(deg) | Heel (s=0) | Mid (s=5)  | Toe (s=10) | Closest Slice');

for (let deg = -12.0; deg <= 0.0; deg += 1.0) {
    ...
    for (let s = 0; s <= numSlices; s++) {
        let minD_s = 999;
        // Test Pinion Flank 1 & 2 against Gear Flank 1 & 2
